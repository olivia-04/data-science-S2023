...
df_q1 <- tibble(x = runif(n, min = 0, max = 1), y = runif(n, min = 0, max = 1)) # Generate the data
```
...
- What is a range of plausible values, based on the sampling
  ...
  - The range of plausible values range between roughly 2.5 and 3.75
...
df_q1 %>%
  ...
  summarise(mean = mean(stat_val), sd = sd(stat_val)) %>% 
  mutate(
    lo = mean - 1.96*sd/sqrt(n),
    up = mean + 1.96*sd/sqrt(n)
  ) %>% 
  glimpse()
```
...
- How closely do your bootstrap CI and CLT CI agree?
  - My bootstrap CI and CLT CI are within .02 of one another
