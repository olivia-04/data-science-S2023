...
-   Are there any sources of *erroneous* uncertainty in the percent occupied you calculated?
    -   Perhaps if I miscounted the unoccupied houses, I also rounded off the percentage at the thousandths place
...
-   Is this sample representative of *all* houses in Helvig? Why or why not?
    -   No as it is only a sample of the houses next to each other. This could cause bias in the data set, for example if families with a shared last name live near one another.
...
-   Which sample---sequential or random---is more *representative* of all homes Helvig? Why?
    -   Random
    -   Sequential sampling has inherent biases as the samples' proximity to one another may be related to factors such as finances, location, relatedness, etc.
...
-   Did you find any highly prevalent names using `df_sample_random` that you *didn't* find in q3 (using `df_sample_seq`)? Write them here.
    -   The top 4 last names in the sequential sample were: Sorensen, Lund, Eklund, and Solberg. The top 4 last names in the random sample are Collins, Blomgren, Regan, and Sorensen. The only shared last name between the two samples is Sorensen.
-   Is there any reason that people with the same last name might tend to *live near each other*?
    -   Perhaps family would want to live in the general vicinity of one another for visiting purposes. Also, people who live near one another may get to know each other and get married.
...
-   Note that we used the *random* sample with the bootstrap procedure in this task. Could we use the bootstrap to make a confidence interval using the sequential sample (`df_sample_seq`) that would be representative of all of Helvig? Why or why not?
    -   No
    -   Though bootstrapping is supposed to mimic the unbiased effects of random sampling, it cannot do this if there is inherent bias 'baked' into the data set being bootstrapped. We have already shown that sequential sampling has biases, and these biases cannot be removed through bootstrapping.
