...
df_q4 <- 
  ...
  mutate(
    ...
    income_lo = income_estimate - zc * income_SE,
    income_hi = income_estimate + zc * income_SE,
  )
```
...
df_data <-
  df_q4 %>% 
  left_join(x = ., y = df_pop, by = "Geography")
```
...
- Document your observations here.
  - The largest confidence interval of median household income is within
    Nantucket 5 person families.
  - Suffolk County has similar median household incomes between family
    sizes.
  - 5 and 6 person families generally have higher median incomes
- Can you confidently distinguish between household incomes in Suffolk
  county? Why or why not?
  - No, as the confidence intervals all overlap between the family sizes
...
- What *overall* trend do you see between `SE` and population? Why might
  this trend exist?
  - As population gets higher, SE gets lower
- What does this *overall* trend tell you about the relative ease of
  studying small vs large counties?
  - Large counties are easier to study than small counties, as the
    former’s SE is smaller
...
df_data %>% 
  ...
  ggplot() +
  geom_col(aes(x = income_estimate, y = geographic_area_name, fill = category), position = 'dodge')
```
...
- Question: How do family sizes affect median income in Rhode Island?

<!-- -->

- Bristol County seems to have the highest median incomes, especially
  among 5 person families. However, the county has the most SE between
  the family sizes.
- Among all the counties, 2 person families have the lowest median
  income
- In Washington County 6 person families have the highest median income,
  which is not true for any of the other counties
- In Providence and Kent Counties, 4 person families have the highest
  median income
- In Bristol and Newport counties, 5 person families have the highest
  median income
- 2 person families have the lowest median income for all counties
