...
df_psaap %>% 
  ggplot() +
  geom_point(aes(x = x, y = T_norm, color = idx))
```
...
- What *Category* of variable is `avg_T`? Why is it such an effective
  ...
  - Avg_T is an output variable. It’s an effective predictor because it
    is an output of the data, as in it is being calculated from the
...
- How do these standard deviations relate to the regression coefficients
  ...
  - The standard deviation of x is higher, and the slope is higher.
    Similarly, the standard deviation of T_f is lower, so the slope is
    lower.
...
- How much do you trust your model? Why?
  - I would trust my model somewhat, as it is based on design
    requirements but there is inherent variability.
